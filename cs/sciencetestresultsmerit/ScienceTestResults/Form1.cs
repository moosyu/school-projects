...
namespace ScienceTestResults
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            if (convertToPercentage(int.Parse(textBoxInput.Text)) != -1)
            {
        ...
        /// <returns>the result converted to a percentage (doesnt include the %) if its in the range</returns>
        private double convertToPercentage(double inputResult)
        {
            if (inputResult >= 0 && inputResult <= 87)
            {
                return ((inputResult / 87) * 100);
            } else
            {
                MessageBox.Show("Please enter a number between 0 and 87 inclusive!");
                return -1;
            }
        }
